...
while True:
    try:
        ...
        for filename in new_files:
            ...
            # ✅ تحويل القيم الرقمية فقط إلى list
            numeric_features = [v for v in features.values() if isinstance(v, (int, float))]

            if len(numeric_features) != 10:
                print(f"[!] Invalid feature vector: expected 10, got {len(numeric_features)} → Skipping file.")
                continue

            # التنبؤ
            proba = model.predict_proba([numeric_features])[0][1]
